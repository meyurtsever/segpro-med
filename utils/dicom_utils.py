...
def load_mammography_series(directory, apply_deidentification=False):
    """
    Load mammography DICOM files from a directory
    
    For mammography, each DICOM file represents a different view (LCC, RCC, LMLO, RMLO)
    rather than sequential slices. We organize them by view position.
    
    Args:
        directory (str): Path to directory containing mammography DICOM files
        apply_deidentification: Whether to apply face removal (typically not applicable to mammography)
        
    Returns:
        tuple: (dict of views with numpy arrays, metadata dict, dict of view->file mapping)
    """
    logger.info(f"Loading mammography series from directory: {directory}")
    ...
    logger.info(f"Found {len(dicom_files)} mammography DICOM files")
    
    # Read all DICOM files and organize by view
    view_data = {}
    view_files = {}
    combined_metadata = {}
    ...
    for file_path in dicom_files:
        try:
            logger.debug(f"Reading mammography DICOM file: {file_path}")
            ds = pydicom.dcmread(file_path)
            
            # Extract view information from filename or DICOM tags
            filename = os.path.basename(file_path)
            view_name = filename.replace('.dcm', '').upper()
            
            # Try to get view position from DICOM tags if available
            if hasattr(ds, 'ViewPosition'):
                view_position = ds.ViewPosition
                if hasattr(ds, 'ImageLaterality'):
                    laterality = ds.ImageLaterality
                    view_name = f"{laterality}{view_position}"
            ...
            view_data[view_name] = pixel_array
            view_files[view_name] = file_path
            
            # Get metadata from first file or merge
            if not combined_metadata:
                combined_metadata = get_dicom_metadata(file_path)
            
            logger.info(f"Loaded view '{view_name}' with shape {pixel_array.shape}")
            
        except Exception as e:
            logger.error(f"Error reading mammography DICOM file {file_path}: {str(e)}")
            continue
    
    if not view_data:
        raise ValueError("Could not read any mammography DICOM files")
    
    logger.info(f"Successfully loaded {len(view_data)} mammography views: {list(view_data.keys())}")
    
    # Add mammography-specific metadata
    if combined_metadata:
        combined_metadata['MammographyInfo'] = {
            'NumberOfViews': len(view_data),
            'ViewNames': list(view_data.keys()),
            'Modality': 'MG',
            'DataType': 'mammography'
        }
    
    return view_data, combined_metadata, view_files

def detect_medical_data_type(directory):
    """
    Detect the type of medical data in a directory based on DICOM modality
    
    Args:
        directory (str): Path to directory containing DICOM files
        
    Returns:
        str: 'mammography', 'brain_mri', 'other_mri', 'ct', 'unknown'
    """
    logger.info(f"Detecting medical data type in directory: {directory}")
    ...
    try:
        ...
        logger.info(f"Detected modality: {modality}, series: {series_description}")
        
        if modality == 'MG':
            return 'mammography'
        elif modality == 'MR':
            if any(brain_keyword in series_description for brain_keyword in ['brain', 'flair', 't1', 't2', 'dwi']):
                return 'brain_mri'
            else:
                return 'other_mri'
        elif modality == 'CT':
