...
# Import brain-specific configurations
from brain_segmentation_configs import get_brain_config, print_config_info
...
# Options: 'high_detail', 'small_structures', 'balanced', 'fast', 'tumor_detection', 'skull_stripping'
BRAIN_CONFIG_MODE = 'fast'  # Change this to experiment with different configurations
...
def main():
    ...
    try:        # Clear any existing Hydra initialization before loading model
        ...
        try:
            ...
            raise        # Create mask generator with optimized parameters for brain structure segmentation
        print("\n🔄 Creating automatic mask generator...")
        print(f"🧠 Using '{BRAIN_CONFIG_MODE}' configuration for brain segmentation:")
        print("   • Skull-stripping and tissue boundary detection")
        print("   • Tumor vs. healthy tissue differentiation") 
        print("   • Lateral ventricle (lvent) segmentation")
        print("   • Third ventricle (tvent) segmentation")
        print("   • Eye and orbital structure detection")
        print("   • Fine anatomical detail preservation")
        
        # Get the selected brain configuration
        brain_config = get_brain_config(BRAIN_CONFIG_MODE)
        print(f"📊 Configuration details:")
        print(f"   • Grid resolution: {brain_config['points_per_side']}x{brain_config['points_per_side']} points")
        print(f"   • IoU threshold: {brain_config['pred_iou_thresh']}")
        print(f"   • Min region area: {brain_config['min_mask_region_area']} pixels")
        print(f"   • Multi-scale crops: {brain_config['crop_n_layers']} layers")
        print(f"   • M2M refinement: {brain_config['use_m2m']}")
        
        generator_start_time = time.time()
        mask_generator = SAM2AutomaticMaskGenerator(
            model=sam2_model,
            **brain_config  # Use the complete configuration
        )
