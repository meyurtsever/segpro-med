...
MAX_SLICES_TO_PROCESS = 10   # Limit number of slices for testing
...
def save_masks_visualization(image, masks, output_path, slice_name):
    """Save comprehensive visualization of generated masks"""
    try:
        if not masks:
        ...
        plt.title(f'Original Image - {slice_name}')
        plt.axis('off')
        ...
        for mask_data in masks:
            all_masks |= mask_data['segmentation']
        
        plt.imshow(display_image, cmap='gray')
        plt.imshow(all_masks, alpha=0.3, cmap='Reds')
        plt.title(f'All Masks Overlay ({len(masks)} masks)')
        plt.axis('off')
        
        # Top 4 masks by IoU
        top_masks = sorted(masks, key=lambda x: x['predicted_iou'], reverse=True)[:4]
        ...
        for i, mask_data in enumerate(top_masks):
            ...
            # Show mask on grayscale background
            plt.imshow(display_image, cmap='gray')
            plt.imshow(mask, alpha=0.5, cmap='Reds')
            plt.title(f'Mask {i+1}\nIoU: {predicted_iou:.3f}, Stab: {stability_score:.3f}\nArea: {area}')
            plt.axis('off')
...
def main():
    ...
    try:        # Clear any existing Hydra initialization before loading model
        ...
        if not SAVE_VISUALIZATIONS:
            print(f"\n💡 Tip: Set SAVE_VISUALIZATIONS = True at the top of the script to save mask visualizations")
            print(f"       (This will make the process slower but provides visual output)")
