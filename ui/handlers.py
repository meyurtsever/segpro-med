...
class ViewerHandlers:
    ...
    def update_slice(self, slider_value, view_type):
        ...
        try:
            ...
            return fig, f"{self.state.current_slice_idx}/{total_slices-1}", crosshair_text, metadata, window_center, window_width
        except Exception as e:
...
class PlotToolHandlers:
    ...
    def update_plot_tool(self, tool_name):
        ...
        self.state.current_plot_tool = plotly_tool
        
        # For reset, return a figure with shapes cleared
        if tool_name == "reset":
            img = display_slice(
                self.state.current_data, 
                self.state.current_slice_idx, 
